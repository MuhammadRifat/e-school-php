...
<section class="container mt-8 text-light">
    <h3 class="text-center border-bottom">Approved Payments</h3>
    <table class="table bgc-secondary rounded text-light text-center my-3" id="table">
        ...
        <tbody>
            ...
            <?php
            $sql_payment = "SELECT id, userEmail, mobile, txnId, date FROM enrolls where status='Approved' order by date desc;";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <td>' . $row['date'] . '</td>
                    <td>
                    <a href="http://localhost/3rd%20year%20project/Online%20Admission%20and%20Learning%20System/admin/approved-payment.php?id=' . $row['id'] . '&notApprove=true" class="btn btn-success btn-sm"><i class="fas fa-edit"></i>Not Approve</a>
                    <a href="http://localhost/3rd%20year%20project/Online%20Admission%20and%20Learning%20System/admin/approved-payment.php?id=' . $row['id'] . '&delete=true" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i> Delete</a>
                    </td>
                </tr>';
                }
            } else {
...
if (isset($_GET['notApprove']) && $_GET['notApprove'] == 'true') {
    $id = $_GET['id'];
...
{
    $sql_approvePayment= "update enrolls set status='Pending' where id = $id";
